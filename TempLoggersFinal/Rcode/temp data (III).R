...
myFolders_years<-list.dirs(path="W:/Fisheries/Fisheries HQ/Cool Water Streams/Monitoring Data/Base Files/",#"H:/Temp Data/", 
                     full.names = TRUE,
                     recursive=FALSE)
...
foreach(s = myStreams) %do% {
  ...
  op$dt <- as.Date(op$dt, format="%m/%d/%Y")
  ...
  p<-ggplot(data=op) +
    ...
    geom_point(aes(x=dt, y=temp, color=tempRange), size=0.3)+
    #geom_point(aes(x=(as.Date(jul, origin=as.date("%m/%d"))), y=temp, color=tempRange), size=.3) +
    #geom_smooth(aes(x=jul, y=temp), color="green", size=1) +
    facet_grid(~site) +
    labs(x="", y="Water Temperature (degrees F)", title="Water Temps", subtitle=s) +
